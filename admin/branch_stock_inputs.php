...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    if ($action === 'approve') { branch_portal_approve_stock_input($id, (int)$u['id'], $note); $msg='Stok masuk disetujui dan sudah masuk stock ledger.'; }
    if ($action === 'reject') { branch_portal_reject_stock_input($id, (int)$u['id'], $note); $msg='Stok masuk ditolak.'; }
...
?>
<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Approval Stok Masuk Cabang</title><link rel="icon" href="<?php echo e(favicon_url()); ?>"><link rel="stylesheet" href="<?php echo e(asset_url('assets/app.css')); ?>"><style><?php echo $customCss; ?></style></head><body><div class="container"><?php include __DIR__ . '/partials_sidebar.php'; ?><div class="main"><div class="topbar"><button class="btn" data-toggle-sidebar type="button">Menu</button></div><div class="content">
<div class="card"><h3>Approval Stok Masuk Cabang</h3><?php if($err): ?><div class="card" style="border-color:#fecdd3;background:#fff1f2;color:#9f1239"><?php echo e($err); ?></div><?php endif; ?><?php if($msg): ?><div class="card" style="border-color:#bbf7d0;background:#f0fdf4;color:#166534"><?php echo e($msg); ?></div><?php endif; ?><form method="get" class="grid cols-4"><div class="row"><label>Cabang</label><select name="branch_id"><option value="0">Semua</option><?php foreach($branches as $b): ?><option value="<?php echo e((string)$b['id']); ?>" <?php echo (int)$b['id']===$branchId?'selected':''; ?>><?php echo e((string)$b['branch_name']); ?></option><?php endforeach; ?></select></div><div class="row"><label>Status</label><select name="status"><option value="">Semua</option><?php foreach(['pending','approved','rejected','cancelled'] as $s): ?><option value="<?php echo e($s); ?>" <?php echo $status===$s?'selected':''; ?>><?php echo e($s); ?></option><?php endforeach; ?></select></div><div class="row" style="align-self:end"><button class="btn" type="submit">Filter</button></div></form></div>
<div class="card"><table class="table"><thead><tr><th>No</th><th>Cabang</th><th>Produk</th><th>Qty</th><th>Input oleh</th><th>Status</th><th>Catatan</th><th>Aksi</th></tr></thead><tbody><?php if(empty($rows)): ?><tr><td colspan="8" style="text-align:center;color:#94a3b8">Tidak ada data.</td></tr><?php else: foreach($rows as $r): $unit=product_unit_fallback($r); ?><tr><td><?php echo e((string)$r['input_no']); ?><br><small><?php echo e((string)$r['created_at']); ?></small></td><td><?php echo e((string)$r['branch_name']); ?></td><td><?php echo e((string)$r['product_name']); ?></td><td><?php echo e(format_qty((float)$r['qty'],$unit['base_unit'])); ?></td><td><?php echo e((string)($r['creator_name'] ?? '-')); ?></td><td><?php echo e((string)$r['status']); ?></td><td><?php echo e((string)($r['notes'] ?? '')); ?></td><td><?php if(($r['status'] ?? '')==='pending'): ?><form method="post" style="display:grid;gap:6px;min-width:220px"><input type="hidden" name="_csrf" value="<?php echo e(csrf_token()); ?>"><input type="hidden" name="id" value="<?php echo e((string)$r['id']); ?>"><input name="approval_note" placeholder="Catatan approval/reject"><div style="display:flex;gap:6px"><button class="btn" name="action" value="approve" type="submit">Approve</button><button class="btn danger" name="action" value="reject" type="submit">Reject</button></div></form><?php else: ?><?php echo e((string)($r['approval_note'] ?? '-')); ?><?php endif; ?></td></tr><?php endforeach; endif; ?></tbody></table></div>
