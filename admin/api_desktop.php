...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'generate') {
    ...
    if ($name === '') {
      ...
      deactivate_active_tokens_by_device($deviceCode);
            ...
            <tr>
              ...
              <td style="display:flex;gap:6px;flex-wrap:wrap">
                ...
                <button
                  ...
                  type="button"
                  data-token-delete
                  data-id="<?php echo e((string)$t['id']); ?>"
                  data-endpoint="<?php echo e(base_url('admin/api/desktop_token_delete.php')); ?>"
...
<script>
document.querySelectorAll('[data-token-delete]').forEach(function(button) {
  button.addEventListener('click', function() {
    var tokenId = parseInt(button.getAttribute('data-id') || '0', 10);
    if (!tokenId) {
      alert('ID token tidak valid.');
      return;
    }
    ...
    formData.append('id', String(tokenId));
    ...
    fetch(button.getAttribute('data-endpoint') || '', {
      ...
      .then(function(payload) {
        if (!payload || payload.ok !== true) {
          throw new Error((payload && payload.message) ? payload.message : 'Gagal menghapus token.');
        }
        window.location.reload();
      })
      .catch(function(error) {
        alert(error && error.message ? error.message : 'Terjadi kesalahan saat menghapus token.');
      });
  });
});
</script>
